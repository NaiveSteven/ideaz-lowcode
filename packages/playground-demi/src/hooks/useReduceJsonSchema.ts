...
import { useWorkspaceStore, useMiddleFormStore } from '../../../playground-store/src';
import {
...
import { isObject, isEmpty } from '@ideal-schema/shared';
...
const getCustomSchemaObject = (
  mode: 'code' | 'preview' = 'code',
  type: 'form' | 'tablePro' = 'form'
) => {
  ...
  if (type === 'tablePro') {
    if (!componentList[0]) return { config: {} };
