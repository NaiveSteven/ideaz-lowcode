...
export default defineComponent({
  ...
  setup() {
    ...
    const handleCopyJSON = () => {
      ...
      const schema = useReduceJsonSchema('code', workspaceComponentType.value);
      copy(
        beautify.js_beautify(
          JSON.stringify(
            workspaceComponentType.value === 'tablePro'
              ? useReduceJsonSchema(schema, workspaceComponentType.value).config
              : useReduceJsonSchema(schema, workspaceComponentType.value)
          ),
          {
            indent_size: 2,
          }
