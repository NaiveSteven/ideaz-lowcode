...
import { useWorkspaceStoreMethods, useHotKeys } from '@/hooks';
import { useWorkspaceStore, useHistoryStore } from '@ideal-schema/playground-store';
...
export function useCommand() {
  const { updateComponentList, updateCurOperateComponent } = useWorkspaceStoreMethods();
  ...
  registry({
    ...
    init() {
      ...
      const start = (type: string) => {
        this.before = cloneDeep(workspaceStore.getWorkspaceComponentList);
    ...
    execute() {
      ...
      const after = cloneDeep(workspaceStore.getWorkspaceComponentList);
      const NOT_UPDATE_OPERATE_LIST = ['复制组件', '表格筛选项拖拽', '表格列拖拽'];
      return {
        before,
        after,
        ...
        redo: () => {
          ...
          // if (!this.historyType || !NOT_UPDATE_OPERATE_LIST.includes(this.historyType)) {
          //   updateCurOperateComponent({} as WorkspaceComponentItem);
          // }
        },
        undo: () => {
  ...
  registry({
    ...
    execute() {
      ...
      const afterAttribute = cloneDeep(workspaceStore.getWorkspaceComponentList);
